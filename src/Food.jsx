...
export default function Food({ data, isSingle }) {
    ...
    return (
        <div className="relative  cursor-pointer group/food flex sm:flex-col h-[5rem] xs:h-32 max-w-full sm:h-auto bg-opacity-20 bg-gray-600 rounded-3xl overflow-hidden text-left">
            ...
            <div className="p-6 box-border">
                <h2 className="text-2xl group-hover/food:tracking-widest transition-all whitespace-nowrap xs:w-60 sm:w-40 lg:w-48 overflow-hidden text-ellipsis">
                    {data.name}
                </h2>
                <p className="h-12 overflow-hidden text-clip hidden xs:block">{data.description}</p>
            </div>
